...
# If the list is greater than 0, send email to receiver & ###########

# Data is available for 9 days including today, so the GET request must be made 9 times for all these dates

#Variables you need to set
receivers = ['viditsinghbrahmania@gmail.com']
dist_code = 651     #You need to find this in the CoWin website - check for the API(Get request) called when search is done
                    #for your district, you'll see the 'district_id'
time_period = 300   #Time in secs after which the script will rerun
...
def check_availibility():
    ...
    for k in available_dates:
        cowinapi = 'https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id=' + str(dist_code) + '&date=' + k
            ...
            for i in vaccination_centers:
                ...
                for j in sessions:
                    if j["available_capacity_dose1"] > 0 and j["min_age_limit"] == 18:
                        slot_data = (
                            i["name"], j["available_capacity_dose1"], j["available_capacity_dose2"],
                            str(i["address"]) + str(i["block_name"]), i["from"],
                            i["to"], k)
